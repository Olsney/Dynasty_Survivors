...
namespace Code.Logic.EnemySpawners
{
    ...
    {
        private const float DarkKnightSpawnIntervalSeconds = 60f;
        ...
        {
            if (_enemyTypeId == EnemyTypeId.DarkKnight)
            {
                _cooldown.SetCooldown(DarkKnightSpawnIntervalSeconds);
                return;
            }
        ...
        {
            if (_enemyTypeId == EnemyTypeId.DarkKnight)
                return DarkKnightSpawnIntervalSeconds;

            return _random.Next(_minSpawnInterval, _maxSpawnInterval);
        }
    }
}
